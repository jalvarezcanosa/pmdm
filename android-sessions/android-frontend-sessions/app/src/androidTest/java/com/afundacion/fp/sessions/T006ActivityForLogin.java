...
public class T006ActivityForLogin extends TestRun {
    ...
    public static void setUp() throws ClassNotFoundException, IOException, InterruptedException {
        ...
        Server.name = Server.name + "/testing/Dev01";
    ...
    public void checkEditTextUsernameIsCorrect() {
        ...
        Espresso.onView(editTextWithHint("jane.doe")).check(isLeftAlignedWith(editTextWithHint("Contraseña")));
        Espresso.onView(editTextWithHint("jane.doe")).check(isRightAlignedWith(editTextWithHint("Contraseña")));
    ...
    public void checkButtonsAreCorrect() {
        Matcher<View> parent = withChild(withText("Login"));
        Espresso.onView(withText("Login")).check(matches(withClassName(Matchers.containsString("Button"))));
        Espresso.onView(withText("Login")).check(isBottomAlignedWith(parent));
        Espresso.onView(withText("Login")).check(isLeftAlignedWith(parent));
        Espresso.onView(withText("Login")).check(isRightAlignedWith(parent));
        Espresso.onView(withText("Login")).check(hasDeclaredId());
        ...
        Espresso.onView(withText("Login")).check(isTopToBottomOf(withText("¿Aún no tienes cuenta?")));
    }
    ...
    public void checkGoToRegisterButtonWorksForthAndBack() throws JSONException {
        ...
        String randUser = "Dev01User" + (new Date()).getTime();
        String randPass = "Dev01Pass" + (new Date()).getTime();
        ...
        Espresso.closeSoftKeyboard();
        Espresso.onView(withText("Crear usuario")).perform(click());
        ...
        Matcher<View> toast = withText(containsString("Cuenta creada con éxito"));
        Espresso.onView(toast).inRoot(isOverlayWindow()).check(matches(isDisplayed()));
        Espresso.onView(withText("Crear usuario")).check(doesNotExist());
    }
